from flask import render_template, url_for, flash, redirect, request, current_app,Blueprint, abort, make_response
...
def user_comments(username):
    page = request.args.get('page', 1, type=int)
    user = User.query.filter_by(username=username).first_or_404()
    comments = Post.query.filter_by(author=user)\
        .order_by(Post.date_posted.desc())\
        .paginate(page=page, per_page=5)
    flash('You are comment.', 'succes')
    return render_template('user_comment.html', comments=comments, user=user)
